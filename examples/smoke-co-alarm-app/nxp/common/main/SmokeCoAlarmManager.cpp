...
void SmokeCoAlarmManager::StartTimer(uint32_t aTimeoutMs)
{
    ...
    {
        ChipLogProgress(DeviceLayer, "Alarm timer already started!");
        CancelTimer();
    }
    ...
    {
        ChipLogError(DeviceLayer, "Alarm timer start() failed");
    }
}
...
void SmokeCoAlarmManager::CancelTimer(void)
{
    ...
    {
        ChipLogError(DeviceLayer, "Alarm timer stop() failed");
    }
}
...
void SmokeCoAlarmManager::TimerEventHandler(TimerHandle_t xTimer)
{
    ...
    SmokeCOAlarmApp::AppTask::GetDefaultInstance().PostEvent(event);
}

void SmokeCoAlarmManager::SelfTestingEventHandler()
{
    ChipLogProgress(DeviceLayer, "Start self-testing!");

    AlarmMgr().mEndSelfTesting = true;
    AlarmMgr().StartTimer(10000); // Self-test simulation in progress
}

void SmokeCoAlarmManager::EndSelfTestingEventHandler(const AppEvent & aEvent)
{
    ...
    ChipLogProgress(DeviceLayer, "End self-testing!");
}
