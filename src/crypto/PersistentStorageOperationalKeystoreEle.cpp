...
void PersistentStorageOperationalKeystore::ResetPendingKey()
{
    ...
    {
        EleManager->EleDeleteKey(mPendingFabricIndex);
    }
...
bool PersistentStorageOperationalKeystore::HasOpKeypairForFabric(FabricIndex fabricIndex) const
{
    ...
    err = hsm_get_key_attr(EleManager->key_mgmt_hdl, &keyattr_args);
    if (err != HSM_NO_ERROR) {
                                                                       ...
                                                                       MutableByteSpan & outCertificateSigningRequest)
{
    ...
    memset(&keyattr_args, 0, sizeof(keyattr_args));
    keyattr_args.key_identifier = fabricIndex;
    hsm_err = hsm_get_key_attr(EleManager->key_mgmt_hdl, &keyattr_args);
    if (hsm_err == HSM_NO_ERROR) {
        ChipLogDetail(Crypto, "Keypair for fabric: %d exists, delete it...\n", fabricIndex);
        EleManager->EleDeleteKey(fabricIndex);
    }
    ...
    hsm_err = hsm_generate_key(EleManager->key_mgmt_hdl, &key_gen_args);
    ...
    err = EleManager->EleGenerateCSR(keyId, outCertificateSigningRequest.data(), csrLength);
                                                                            ...
                                                                            const Crypto::P256PublicKey & nocPublicKey)
{
    ...
    hsmret = hsm_pub_key_recovery(EleManager->key_store_hdl, &args);
...
CHIP_ERROR PersistentStorageOperationalKeystore::CommitOpKeypairForFabric(FabricIndex fabricIndex)
{
    ...
    hsmret = hsm_manage_key_group(EleManager->key_mgmt_hdl, &keygroup_args);
    if (hsmret != HSM_NO_ERROR) {
...
CHIP_ERROR PersistentStorageOperationalKeystore::RemoveOpKeypairForFabric(FabricIndex fabricIndex)
{
    ...
    memset(&keyattr_args, 0, sizeof(keyattr_args));
    keyattr_args.key_identifier = fabricIndex;
    hsmret = hsm_get_key_attr(EleManager->key_mgmt_hdl, &keyattr_args);
    if (hsmret != HSM_NO_ERROR) {
        ChipLogDetail(Crypto, "No keypair for fabric: %d found. No need to delete\n", fabricIndex);
        return CHIP_NO_ERROR;
    } else {
        hsmret = EleManager->EleDeleteKey(fabricIndex);
        if (hsmret != HSM_NO_ERROR)
            return CHIP_ERROR_HSM;
    }
                                                                   ...
                                                                   Crypto::P256ECDSASignature & outSignature) const
{
    ...
    hsmret = EleManager->EleSignMessage(fabricIndex, message.data(), message.size(), sig, kP256_ECDSA_Signature_Length_Raw);
