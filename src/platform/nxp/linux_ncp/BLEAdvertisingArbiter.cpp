...
{
    // Note: ncp_bt_le_adv_stop() returns success when the advertising was not started
    ReturnErrorOnFailure(MapErrorZephyr(ncp_bt_le_adv_stop()));
    ReturnErrorCodeIf(sys_slist_is_empty(&sRequests), CHIP_NO_ERROR);

    const Request & top = ToRequest(sys_slist_peek_head(&sRequests));
    const int error = ncp_bt_set_adv_data(top.advertisingData.data(), top.advertisingData.size());
    if (error != 0)
    {
        ChipLogProgress(DeviceLayer, "ncp_bt_set_adv_data: set adv data fail");
    }

    const int result = ncp_bt_le_adv_start();
    ChipLogProgress(DeviceLayer, "ncp_bt_le_adv_start: start adv");
