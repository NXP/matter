...
CHIP_ERROR LayerImplSelect::StartWatchingSocket(int fd, SocketWatchToken * tokenOut)
{
    ...
    for (auto & w : mSocketWatchPool)
    {
        if (w.mFD == fd)
        {
            ChipLogError(DeviceLayer, "select fd already set");
...
{
#if 1
    mSelectResult = ncp_select(mMaxFd + 1, &mSelected.mReadSet, &mSelected.mWriteSet, &mSelected.mErrorSet, &mNextTimeout);
#else
