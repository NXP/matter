...
CHIP_ERROR TCPEndPointImplNcp::BindImpl(IPAddressType addrType, const IPAddress & addr, uint16_t port, bool reuseAddr)
{
    ...
    {
        int n = 1;
        ncp_setsockopt(mSocket, SOL_SOCKET, SO_REUSEADDR, &n, sizeof(n));
        ...
        if (ncp_setsockopt(mSocket, SOL_SOCKET, SO_REUSEPORT, &n, sizeof(n)) != 0)
        {
    ...
    if (res == CHIP_NO_ERROR)
    {
        ...
        if (res == CHIP_NO_ERROR)
        {
            if (ncp_bind(mSocket, &sa.any, sockaddrsize) != 0)
            {
...
CHIP_ERROR TCPEndPointImplNcp::ListenImpl(uint16_t backlog)
{
    if (ncp_listen(mSocket, backlog) != 0)
    {
        return CHIP_ERROR_POSIX(errno);
    }

    // Enable non-blocking mode for the socket.
    int flags = ncp_fcntl(mSocket, F_GETFL, 0);
    ncp_fcntl(mSocket, F_SETFL, flags | O_NONBLOCK);
...
CHIP_ERROR TCPEndPointImplNcp::ConnectImpl(const IPAddress & addr, uint16_t port, InterfaceId intfId)
{
...
CHIP_ERROR TCPEndPointImplNcp::GetPeerInfo(IPAddress * retAddr, uint16_t * retPort) const
{
    return GetSocketInfo(getpeername, retAddr, retPort);
}

CHIP_ERROR TCPEndPointImplNcp::GetLocalInfo(IPAddress * retAddr, uint16_t * retPort) const
{
    return GetSocketInfo(getsockname, retAddr, retPort);
}

CHIP_ERROR TCPEndPointImplNcp::GetSocketInfo(int getname(int, sockaddr *, socklen_t *), IPAddress * retAddr,
...
CHIP_ERROR TCPEndPointImplNcp::GetInterfaceId(InterfaceId * retInterface)
{
    VerifyOrReturnError(IsConnected(), CHIP_ERROR_INCORRECT_STATE);
    ...
    if (ncp_getpeername(mSocket, &sa.any, &saLen) != 0)
    {
        return CHIP_ERROR_POSIX(errno);
    }
...
CHIP_ERROR TCPEndPointImplNcp::SendQueuedImpl(bool queueWasEmpty)
{
    if (queueWasEmpty)
    {
        // Wait for ability to write on this endpoint.
...
CHIP_ERROR TCPEndPointImplNcp::EnableNoDelay()
{
    VerifyOrReturnError(IsConnected(), CHIP_ERROR_INCORRECT_STATE);
    ...
    int val = 1;
    if (ncp_setsockopt(mSocket, TCP_SOCKOPT_LEVEL, TCP_NODELAY, &val, sizeof(val)) != 0)
    {
        return CHIP_ERROR_POSIX(errno);
    }
...
CHIP_ERROR TCPEndPointImplNcp::EnableKeepAlive(uint16_t interval, uint16_t timeoutCount)
{
    VerifyOrReturnError(IsConnected(), CHIP_ERROR_INCORRECT_STATE);

    // Set the idle interval
    int val = interval;
    if (ncp_setsockopt(mSocket, TCP_SOCKOPT_LEVEL, TCP_IDLE_INTERVAL_OPT_NAME, &val, sizeof(val)) != 0)
    {
        return CHIP_ERROR_POSIX(errno);
    }
    ...
    val = interval;
    if (ncp_setsockopt(mSocket, TCP_SOCKOPT_LEVEL, TCP_KEEPINTVL, &val, sizeof(val)) != 0)
    {
        return CHIP_ERROR_POSIX(errno);
    }
    ...
    val = timeoutCount;
    if (ncp_setsockopt(mSocket, TCP_SOCKOPT_LEVEL, TCP_KEEPCNT, &val, sizeof(val)) != 0)
    {
        return CHIP_ERROR_POSIX(errno);
    }
    ...
    val = 1; // enable
    if (ncp_setsockopt(mSocket, SOL_SOCKET, SO_KEEPALIVE, &val, sizeof(val)) != 0)
    {
        return CHIP_ERROR_POSIX(errno);
    }
...
CHIP_ERROR TCPEndPointImplNcp::DisableKeepAlive()
{
    VerifyOrReturnError(IsConnected(), CHIP_ERROR_INCORRECT_STATE);
...
CHIP_ERROR TCPEndPointImplNcp::AckReceive(uint16_t len)
{
    VerifyOrReturnError(IsConnected(), CHIP_ERROR_INCORRECT_STATE);
...
CHIP_ERROR TCPEndPointImplNcp::SetUserTimeoutImpl(uint32_t userTimeoutMillis)
{
#if defined(TCP_USER_TIMEOUT)
    // Set the user timeout
    uint32_t val = userTimeoutMillis;
    if (ncp_setsockopt(mSocket, TCP_SOCKOPT_LEVEL, TCP_USER_TIMEOUT, &val, sizeof(val)) != 0)
    {
        return CHIP_ERROR_POSIX(errno);
    }
    return CHIP_NO_ERROR;
...
CHIP_ERROR TCPEndPointImplNcp::DriveSendingImpl()
{
    ...
    while (!mSendQueue.IsNull())
    {
        ...
        ssize_t lenSentRaw = ncp_send(mSocket, mSendQueue->Start(), bufLen, sendFlags);
    ...
    if (err == CHIP_NO_ERROR)
    {
        ...
        {
            if (ncp_shutdown(mSocket, SHUT_WR) != 0)
            {
...
void TCPEndPointImplNcp::HandleConnectCompleteImpl()
{
...
void TCPEndPointImplNcp::DoCloseImpl(CHIP_ERROR err, State oldState)
{
...
#if INET_CONFIG_OVERRIDE_SYSTEM_TCP_USER_TIMEOUT
void TCPEndPointImplNcp::TCPUserTimeoutHandler()
{
...
CHIP_ERROR TCPEndPointImplNcp::BindSrcAddrFromIntf(IPAddressType addrType, InterfaceId intfId)
{
...
CHIP_ERROR TCPEndPointImplNcp::GetSocket(IPAddressType addrType)
{
    if (mSocket == kInvalidSocketFd)
    {
        int family;
        ...
        mSocket = ::ncp_socket(family, SOCK_STREAM | SOCK_CLOEXEC, 0);
        if (mSocket == -1)
        {
            return CHIP_ERROR_POSIX(errno);
        }
        ...
        {
            int one = 1;
            ncp_setsockopt(mSocket, IPPROTO_IPV6, IPV6_V6ONLY, &one, sizeof(one));
        }
        ...
        {
            int one = 1;
            int res = ncp_setsockopt(mSocket, SOL_SOCKET, SO_NOSIGPIPE, &one, sizeof(one));
            if (res != 0)
            {
...
// static
void TCPEndPointImplNcp::HandlePendingIO(System::SocketEvents events, intptr_t data)
{
    reinterpret_cast<TCPEndPointImplNcp *>(data)->HandlePendingIO(events);
}

void TCPEndPointImplNcp::HandlePendingIO(System::SocketEvents events)
{
    ...
    else if (mState == State::kConnecting)
    {
        ...
        if (events.Has(System::SocketEventFlags::kWrite))
        {
            ...
            if (ncp_getsockopt(mSocket, SOL_SOCKET, SO_ERROR, &osConRes, &optLen) != 0)
            {
                osConRes = errno;
            }
#else  // __MBED__
...
void TCPEndPointImplNcp::ReceiveData()
{
...
void TCPEndPointImplNcp::HandleIncomingConnection()
{
    CHIP_ERROR err                 = CHIP_NO_ERROR;
    TCPEndPointImplNcp * conEP = nullptr;
    IPAddress peerAddr;
    uint16_t peerPort;
    ...
    int conSocket = ncp_accept(mSocket, &sa.any, &saLen);
    if (conSocket == -1)
    {
        if (errno == EAGAIN || errno == EWOULDBLOCK)
        {
    ...
    if (err == CHIP_NO_ERROR)
    {
        ...
        conEP                         = static_cast<TCPEndPointImplNcp *>(connectEndPoint);
    }
 ...
 */
CHIP_ERROR TCPEndPointImplNcp::CheckConnectionProgress(bool & isProgressing)
{
    ...
    if (ncp_getsockopt(mSocket, SOL_SOCKET, SO_NWRITE, &currPendingBytesRaw, &len) < 0)
#else
    if (ncp_ioctl(mSocket, TIOCOUTQ, &currPendingBytesRaw) < 0)
#endif
    {
        return CHIP_ERROR_POSIX(errno);
    }
