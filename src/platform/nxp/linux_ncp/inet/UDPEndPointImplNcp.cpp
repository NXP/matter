 ...
 * definitions.
 */
#define LWIP_IPV6_JOIN_GROUP      12
#define INET_IPV6_ADD_MEMBERSHIP  LWIP_IPV6_JOIN_GROUP
#define LWIP_IPV6_LEAVE_GROUP     13
#define INET_IPV6_DROP_MEMBERSHIP LWIP_IPV6_LEAVE_GROUP

#define LWIP_IP_ADD_MEMBERSHIP    3
#define INET_IP_ADD_MEMBERSHIP    LWIP_IP_ADD_MEMBERSHIP
#define LWIP_IP_DROP_MEMBERSHIP   4
#define INET_IP_DROP_MEMBERSHIP   LWIP_IP_DROP_MEMBERSHIP

#define HAVE_IPV6_MULTICAST       1
#define NCP_DEVICE_STA_NETIF_INDEX       2
...
CHIP_ERROR IPv6Bind(int socket, const IPAddress & address, uint16_t port, InterfaceId interface)
{
    ...
    ncp_setsockopt(socket, IPPROTO_IPV6, IPV6_MULTICAST_HOPS, &hops, sizeof(hops));
...
CHIP_ERROR IPv4Bind(int socket, const IPAddress & address, uint16_t port)
{
    ...
    else
    {
        ...
        ncp_setsockopt(socket, SOL_SOCKET, SO_BROADCAST, &enable, sizeof(enable));

#ifdef IP_MULTICAST_IF
        // Instruct the kernel that any messages to multicast destinations should be
        // sent down the interface to which the specified IPv4 address is bound.
        ncp_setsockopt(socket, IPPROTO_IP, IP_MULTICAST_IF, &sa, sizeof(sa));
    ...
    ncp_setsockopt(socket, IPPROTO_IP, IP_MULTICAST_TTL, &ttl, sizeof(ttl));
...
UDPEndPointImplNcp::MulticastGroupHandler UDPEndPointImplNcp::sMulticastGroupHandler;
#endif // CHIP_SYSTEM_CONFIG_USE_PLATFORM_MULTICAST_API

CHIP_ERROR UDPEndPointImplNcp::BindImpl(IPAddressType addressType, const IPAddress & addr, uint16_t port, InterfaceId interface)
{
    ...
    if (port == 0)
    {
        ...
        if (ncp_getsockname(mSocket, &boundAddr.any, &boundAddrLen) == 0)
        {
...
CHIP_ERROR UDPEndPointImplNcp::BindInterfaceImpl(IPAddressType addressType, InterfaceId interfaceId)
{
    // Make sure we have the appropriate type of socket.
    ReturnErrorOnFailure(GetSocket(addressType));
...
InterfaceId UDPEndPointImplNcp::GetBoundInterface() const
{
    return mBoundIntfId;
}

uint16_t UDPEndPointImplNcp::GetBoundPort() const
{
    return mBoundPort;
}

CHIP_ERROR UDPEndPointImplNcp::ListenImpl()
{
...
CHIP_ERROR UDPEndPointImplNcp::SendMsgImpl(const IPPacketInfo * aPktInfo, System::PacketBufferHandle && msg)
{
    ...
    unsigned char *send_buf = msg->Start();
    int send_len  = msg->DataLength();
    ...
    const ssize_t lenSent = ncp_sendto(mSocket, send_buf, send_len, 0, (struct sockaddr *)&peerSockAddr, namelen);
    if (lenSent == -1)
    {
...
void UDPEndPointImplNcp::CloseImpl()
{
...
void UDPEndPointImplNcp::Free()
{
    Close();
    Release();
}

CHIP_ERROR UDPEndPointImplNcp::GetSocket(IPAddressType addressType)
{
    if (mSocket == kInvalidSocketFd)
    {
        ...
        mSocket = ::ncp_socket(family, type, protocol);
        if (mSocket == -1)
            return CHIP_ERROR_POSIX(errno);
        ...
        constexpr int one = 1;
        int res           = ncp_setsockopt(mSocket, SOL_SOCKET, SO_REUSEADDR, &one, sizeof(one));
...
#ifdef UNDEFINE
        res = ncp_setsockopt(mSocket, SOL_SOCKET, SO_REUSEPORT, &one, sizeof(one));
        if (res != 0)
        {
        ...
        if (addressType == IPAddressType::kIPv6)
        {
            res = ncp_setsockopt(mSocket, IPPROTO_IPV6, IPV6_V6ONLY, &one, sizeof(one));
            if (res != 0)
            {
        ...
        {
            res = ncp_setsockopt(mSocket, IPPROTO_IP, IP_PKTINFO, &one, sizeof(one));
            if (res != 0)
            {
        ...
        if (addressType == IPAddressType::kIPv6)
        {
            res = ncp_setsockopt(mSocket, IPPROTO_IPV6, IPV6_RECVPKTINFO, &one, sizeof(one));
            if (res != 0)
            {
...
#ifdef UNDEFINE
        {
            res = ncp_setsockopt(mSocket, SOL_SOCKET, SO_NOSIGPIPE, &one, sizeof(one));
            if (res != 0)
            {
...
// static
void UDPEndPointImplNcp::HandlePendingIO(System::SocketEvents events, intptr_t data)
{
    reinterpret_cast<UDPEndPointImplNcp *>(data)->HandlePendingIO(events);
}
...
static CHIP_ERROR SocketsSetMulticastLoopback(int aSocket, bool aLoopback, int aProtocol, int aOption)
{
    ...
    if (ncp_setsockopt(aSocket, aProtocol, aOption, &lValue, sizeof(lValue)) != 0)
    {
        return CHIP_ERROR_POSIX(errno);
    }
...
CHIP_ERROR UDPEndPointImplNcp::SetMulticastLoopback(IPVersion aIPVersion, bool aLoopback)
{
...
#if INET_CONFIG_ENABLE_IPV4
CHIP_ERROR UDPEndPointImplNcp::IPv4JoinLeaveMulticastGroupImpl(InterfaceId aInterfaceId, const IPAddress & aAddress, bool join)
{
    ...
    const int command = join ? INET_IP_ADD_MEMBERSHIP : INET_IP_DROP_MEMBERSHIP;
    if (ncp_setsockopt(mSocket, IPPROTO_IP, command, &lMulticastRequest, sizeof(lMulticastRequest)) != 0)
    {
        return CHIP_ERROR_POSIX(errno);
    }
    return CHIP_NO_ERROR;
...
CHIP_ERROR UDPEndPointImplNcp::IPv6JoinLeaveMulticastGroupImpl(InterfaceId aInterfaceId, const IPAddress & aAddress, bool join)
{
    ...
    memset(&lMulticastRequest, 0, sizeof(lMulticastRequest));
    lMulticastRequest.ipv6mr_interface = NCP_DEVICE_STA_NETIF_INDEX;
    ...
    if (ncp_setsockopt(mSocket, IPPROTO_IPV6, command, &lMulticastRequest, sizeof(lMulticastRequest)) != 0)
    {
        return CHIP_ERROR_POSIX(errno);
    }
    return CHIP_NO_ERROR;
